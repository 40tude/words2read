...
// Display the number of words which will appear in the blog post
//
...
fn count_words_to_read(md_path: &str, word_limit: usize) -> String {
    ...
    let mut report = format!("Words to read : {word_count}");

    if word_count > word_limit {
        let remaining_words = &words[word_limit..];
        let paragraph_start: String = remaining_words.iter().take(50).cloned().collect::<Vec<&str>>().join(" ");
...
fn main() {
    ...
    let result = count_words_to_read(&args[1], 3000);
    println!("{result}");
}
